...
const semver = { compare: semverCompare, gte: semverGte, lt: semverLt };
...
function getSupportInfo({
  ...
  const options = arrayify(
    ...
    .map((option) => {
      ...
      const pluginDefaults = fromPairs(
        plugins
          .filter(
            (plugin) => plugin.defaultOptions && plugin.defaultOptions[option.name] !== undefined,
          )
  ...
  function filterSince(object) {
    return (
      showUnreleased || !('since' in object) || (object.since && semver.gte(version, object.since))
    );
  }
