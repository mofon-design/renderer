...
//     ...
//   }
function handleIfStatementComments({ comment, precedingNode, enclosingNode, followingNode, text }) {
...
function handleWhileComments({ comment, precedingNode, enclosingNode, followingNode, text }) {
...
function handleTryStatementComments({ comment, precedingNode, enclosingNode, followingNode }) {
  if (
    !enclosingNode ||
    (enclosingNode.type !== 'TryStatement' && enclosingNode.type !== 'CatchClause') ||
    !followingNode
  ) {
    return false;
  }
...
function handleMemberExpressionComments({ comment, enclosingNode, followingNode }) {
...
function handleConditionalExpressionComments({
  ...
  const isSameLineAsPrecedingNode =
    precedingNode && !hasNewlineInRange(text, locEnd(precedingNode), locStart(comment));
...
function handleObjectPropertyAssignment({ comment, precedingNode, enclosingNode }) {
  if (
    enclosingNode &&
...
function handleClassComments({ comment, precedingNode, enclosingNode, followingNode }) {
  if (
    enclosingNode &&
...
function handleMethodNameComments({ comment, precedingNode, enclosingNode, text }) {
...
function handleFunctionNameComments({ comment, precedingNode, enclosingNode, text }) {
...
function handleCommentInEmptyParens({ comment, enclosingNode, text }) {
  ...
  if (
    enclosingNode &&
    ((isRealFunctionLikeNode(enclosingNode) && getFunctionParameters(enclosingNode).length === 0) ||
      (isCallLikeExpression(enclosingNode) && getCallArguments(enclosingNode).length === 0))
  ) {
    addDanglingComment(enclosingNode, comment);
    return true;
  }
...
function handleLastFunctionArgComments({
  ...
  if (
    precedingNode &&
    (precedingNode.type === 'Identifier' || precedingNode.type === 'AssignmentPattern') &&
    enclosingNode &&
  ...
  if (
    ...
    const functionParamRightParenIndex = (() => {
      ...
      return (
        ...
        getNextNonSpaceNonCommentCharacterIndexWithStartIndex(text, functionParamLeftParenIndex + 1)
      );
    })();
...
function handleBreakAndContinueStatementComments({ comment, enclosingNode }) {
  if (
    enclosingNode &&
    (enclosingNode.type === 'ContinueStatement' || enclosingNode.type === 'BreakStatement') &&
    !enclosingNode.label
  ) {
...
function handleUnionTypeComments({ comment, precedingNode, enclosingNode, followingNode }) {
  if (
    enclosingNode &&
    (enclosingNode.type === 'UnionTypeAnnotation' || enclosingNode.type === 'TSUnionType')
  ) {
  ...
  if (
    followingNode &&
    (followingNode.type === 'UnionTypeAnnotation' || followingNode.type === 'TSUnionType') &&
...
function handleOnlyComments({ comment, enclosingNode, followingNode, ast, isLastComment }) {
...
function handleForComments({ comment, enclosingNode }) {
  if (
    enclosingNode &&
    (enclosingNode.type === 'ForInStatement' || enclosingNode.type === 'ForOfStatement')
  ) {
    addLeadingComment(enclosingNode, comment);
    return true;
  }
...
function handleImportDeclarationComments({ comment, precedingNode, enclosingNode, text }) {
  if (
    precedingNode &&
...
function handleVariableDeclaratorComments({ comment, enclosingNode, followingNode }) {
  if (
    enclosingNode &&
...
function handleTSFunctionTrailingComments({ comment, enclosingNode, followingNode, text }) {
  if (
    !followingNode &&
    enclosingNode &&
...
function handleTSMappedTypeComments({ comment, precedingNode, enclosingNode, followingNode }) {
