...
function hasFlowAnnotationComment(comments) {
  return comments && isBlockComment(comments[0]) && FLOW_ANNOTATION.test(comments[0].value);
}
...
function isObjectTypePropertyAFunction(node) {
  return (
    (node.type === 'ObjectTypeProperty' || node.type === 'ObjectTypeInternalSlot') &&
...
function isSkipOrOnlyBlock(node) {
  return (
    ...
    (node.callee.property.name === 'only' || node.callee.property.name === 'skip')
  );
}
...
function isFlowAnnotationComment(text, typeAnnotation) {
  ...
  return (
    end !== false && text.slice(start, start + 2) === '/*' && text.slice(end, end + 2) === '*/'
  );
}
...
function isTemplateOnItsOwnLine(node, text) {
  return (
    ((node.type === 'TemplateLiteral' && templateLiteralHasNewLines(node)) ||
      (node.type === 'TaggedTemplateExpression' && templateLiteralHasNewLines(node.quasi))) &&
...
function startsWithNoLookaheadToken(node, forbidFunctionClassAndDoExpr) {
  ...
  switch (node.type) {
    ...
    case 'UpdateExpression':
      return (
        !node.prefix && startsWithNoLookaheadToken(node.argument, forbidFunctionClassAndDoExpr)
      );
    case 'BindExpression':
...
function isBitwiseOperator(operator) {
  return (
    Boolean(bitshiftOperators[operator]) || operator === '|' || operator === '^' || operator === '&'
  );
}
...
function isCallLikeExpression(node) {
  return (
    isCallExpression(node) || node.type === 'NewExpression' || node.type === 'ImportExpression'
  );
}
